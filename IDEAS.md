...
- **Vendor Chart.js locally instead of the jsDelivr CDN** — the app currently can't draw a single chart with no internet, which is a real limitation for a desktop journal (and the fonts have the same dependency). Ship the ~200KB UMD build in the repo.
- **Give Chart.js font families a fallback** — every chart config asks for bare `'JetBrains Mono'` with no `, monospace` after it, so if the webfont hasn't loaded the canvas text silently falls back to a *serif* face that clashes with the whole UI. Observed in offline rendering.
