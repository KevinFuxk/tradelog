...
- **R precision + a latent drop-out bug** — `tradeR` divided by `(actualRiskPct / 100) * accountBalance`, re-deriving the risk from a value `calcRisk` had already rounded to 2 decimals for display. Two consequences: every R inherited that rounding (measured up to ~1.6% off on tight-stop trades), and — worse — a trade whose risk rounds to `0.00%` failed the `actualRiskPct > 0` guard, so its R-multiple was **silently dropped everywhere** (table sub-line, journal header, Avg R, CSV) despite being perfectly computable. `calcRisk` now also returns the exact `dollarRisk` and `tradeR` divides by that. Purely additive to the returned object; nothing persisted changes.
- **Journal header readability** — the R-multiple and the `N/6 sections` counter were `--tx3` (#3d4f6b on #0d1117 ≈ **2.3:1**), dimmer than the timestamp beside them, so the *most* useful number in the row was the hardest to read. Both moved to `--tx2` (≈ **5.3:1**, WCAG AA). Inverted hierarchy fixed.
- **Removed a re-implementation of `avgTradeR`** inside `renderDash` (the Avg R card had inlined the same map/filter/reduce), so the average-R logic is now single-source too, not just the per-trade formula.

- **CRITICAL: the app was unusable without internet.** Running the real Electron build offline showed the CDN `<script>` for Chart.js failing, leaving `Chart` undefined. `renderDash` then threw `ReferenceError: Chart is not defined` inside `buildCharts`, which aborted `renderAll()` — and because `boot()` calls `wire()` *after* `renderAll()`, no event listeners were ever attached. Result: stat cards rendered, but the trades table, journal and strategy views were all empty and **the navigation was completely dead**. Your data was intact on disk and totally unreachable. Fixed by vendoring Chart.js to `src/vendor/chart.umd.min.js` (no network needed) and adding a `drawCharts()` wrapper so a chart failure can never take the rest of the UI down again.
- **Startup blocked ~12.6s on the Google Fonts stylesheet.** A render-blocking `<link rel=stylesheet>` also blocks execution of the inline `<script>` beneath it, so whenever `fonts.googleapis.com` is slow or unreachable the whole app sat blank until the request timed out — measured at **12,650ms**, three runs running. Now loaded non-blocking (`media="print"` + `onload="this.media='all'"`); every font stack already had a `monospace`/`sans-serif` fallback, so text paints immediately and the webfont swaps in when it arrives. Measured after: **~250ms** (~50x faster cold start).

### Deferred / next-up ideas (2026-09-02)
- **Self-host the two webfonts** — they're non-blocking now, but on a firewalled network you still get fallback typography forever. Shipping the woff2 files would make the app fully self-contained.
- **Give Chart.js font families a fallback** — every chart config asks for bare `'JetBrains Mono'` with no `, monospace` after it, so if the webfont hasn't loaded the canvas text falls back to a *serif* face that clashes with the UI. Cosmetic-only now that startup no longer waits on fonts.
- **Drop the "Break Even" key from the Win/Loss donut when the count is 0** — the legend currently advertises a segment that isn't in the chart.
- **Sortable Hold Time column in Trades table** — still pending a responsive width pass before adding another column
